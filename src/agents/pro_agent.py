...
def argue_pro(claim: str, evidence_pool: Dict[str, Dict[str, Any]], prior_turns: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Executes a Pro debater turn with rate-limit resilient Groq client calls.
    """
    # 1. Reformulate stance-conditioned query
    query = f"{claim} supporting evidence studies research benefits true"
    ...
    prompt = get_debater_prompt(stance="pro", claim=claim, evidence_text=evidence_text, prior_transcript=prior_transcript)
    ...
    if GROQ_API_KEY:
        try:
            ...
            logger.warning(f"Groq pro_agent call failed: {e}. Using structured fallback.")
    ...
    fallback_arg = (
        f"Empirical evidence directly supports the proposition '{claim}'. "
        f"Key scientific findings demonstrate robust positive indicators {citations_str}. "
        f"Furthermore, cohort analyses confirm that under documented parameters, the claim holds true."
    )
    return {
        "role": "pro",
